...
if configFile then
    ...
    print("No se pudo cargar config.lua")
end
...
RegisterServerEvent('kw_coords:getDiscordAvatar')
AddEventHandler('kw_coords:getDiscordAvatar', function()
    local src = source
    local avatarURL = GetDiscordAvatar(src)
    if avatarURL then
        TriggerClientEvent('kw_coords:sendDiscordAvatar', src, avatarURL)
    end
end)
